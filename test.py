...
DEFAULT_TEST_CASE = TestCase(argv=[], stdin=bytes(
), returncode=0, stdout=bytes(), stderr=bytes())
    ...
    try:
        with open(file_path, 'rb') as f:
            ...
            for index in range(argc):
                argv.append(read_blob_field(f, b'arg%d' %
                            index).decode('utf-8'))
...
def run_test_for_file(file_path: str, stats: RunStats = RunStats()):
    ...
    if tc is not None:
        com = cmd_run_echoed(['./boba', '--run', '--silent', file_path,
        ...
        com = cmd_run_echoed(['./boba', '--silent', file_path])
        if com.returncode != 0:
            error = True
            stats.failed += 1
        stats.ignored += 1
...
def update_output_for_file(file_path: str):
    ...
    output = cmd_run_echoed(['./boba', '--run', '--silent',
...
if __name__ == '__main__':
    ...
    if subcommand == 'update' or subcommand == 'record':
        ...
        if subsubcommand == 'output':
            ...
            if len(argv) == 0:
                usage(exe_name)
                print('[ERROR] no file is provided for `%s %s` subcommand' %
                      (subcommand, subsubcommand), file=sys.stderr)
                exit(1)
            file_path, *argv = argv
            update_input_for_file(file_path, argv)
        else:
            usage(exe_name)
            print('[ERROR] unknown subcommand `%s %s`. Available commands are `%s input` or `%s output`' % (
                subcommand, subsubcommand, subcommand, subcommand), file=sys.stderr)
            exit(1)
        ...
        if path.isdir(target):
            ...
            print('[ERROR] the path `%s` does not exist' % target,
                  file=sys.stderr)
            exit(1)
